  ...
  <div class="container">
    ...
    <div class="card">
      <div class="m-profile">
        ...
        <textarea class="textarea" placeholder="あなたを紹介する説明文" v-model="description">{{ user_data.description }}</textarea>
        <br>
      ...
      <footer>
        <div class="field is-grouped is-grouped-centered">
          <p class="control">
            <a class="button is-primary" v-on:click="onSubmit">
              保存
            </a>
          </p>
        </div>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      //axiosでサーバーに送信
      axios.post('/api/users/profile/update',data)
      .then((response) => {
        console.log(response.data);
        this.$emit('shownotification','プロフィールの更新に成功しました!','is-success')
        this.is_uploading = false;
        this.$emit('tglloading', 'プロフィール設定中');
      })
      .catch((error) => {
        console.log(error.response);
        this.$emit('shownotification',"プロフィールの更新に失敗しました…("+error+" "+error.response.data+")",'is-danger')
        this.is_uploading = false;
        this.$emit('tglloading', 'プロフィール設定中');
      });
    },
